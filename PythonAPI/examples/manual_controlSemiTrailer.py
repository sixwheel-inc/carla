...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        # Sensors list to hold multiple camera managers
    ...
    def restart(self):
        ...
        blueprint = random.choice(get_actor_blueprints(self.world, "DAFxf", "2"))
        ...
        blueprintTrailer = random.choice(get_actor_blueprints(self.world, "trailer", "2"))
        ...
        # Initialize cameras around the trailer
        camera_transforms = [
            # Front camera
            carla.Transform(
        ...
        # Clear existing camera managers
        self.camera_managers = []

        for transform in camera_transforms:
            camera_manager = CameraManager(self.playerTrailer, self.hud, 2.2)
            camera_manager.set_sensor(0, notify=False)
            camera_manager.sensor.set_transform(transform)
    ...
    def render(self, display):
        ...
        # Calculate dimensions for each camera feed
        width = int(self.hud.dim[0] / 2)
        height = int(self.hud.dim[1] / 2)
        positions = [
            (0, 0),            # Top-left
            (width, 0),        # Top-right
        ...
        # Render each camera view in the grid
        for i, camera_manager in enumerate(self.camera_managers):
            if camera_manager.sensor is not None and camera_manager.surface is not None:
                # Resize and blit the camera surface
                surface = pygame.transform.scale(camera_manager.surface, (width, height))
                display.blit(surface, positions[i])
...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        self.sensors = [
            ['sensor.camera.rgb', cc.Raw, 'Camera RGB Fisheye', {}],
            ['sensor.camera.depth', cc.Raw, 'Camera Depth Fisheye (Raw)', {}],
            ['sensor.camera.depth', cc.Depth, 'Camera Depth Fisheye (Gray Scale)', {}],
            ['sensor.camera.depth', cc.LogarithmicDepth, 'Camera Depth Fisheye (Logarithmic Gray Scale)', {}],
            ['sensor.camera.semantic_segmentation', cc.Raw, 'Camera Semantic Segmentation Fisheye (Raw)', {}],
            ['sensor.camera.semantic_segmentation', cc.CityScapesPalette, 'Camera Semantic Segmentation Fisheye (CityScapes Palette)', {}],
            ['sensor.camera.instance_segmentation', cc.CityScapesPalette, 'Camera Instance Segmentation Fisheye (CityScapes Palette)', {}],
            ['sensor.camera.instance_segmentation', cc.Raw, 'Camera Instance Segmentation Fisheye (Raw)', {}],
    ...
    def set_sensor(self, index, notify=True, force_respawn=False):
        ...
        if needs_respawn:
            ...
            # Get the blueprint and apply custom attributes
            bp = self.sensors[index][-1]
            
            # Set FOV to match H190X specs
            if bp.has_attribute('hfov'):
                print(bp.get_attribute('hfov'))
                bp.set_attribute('fov', '190.0')  # Horizontal FOV
            # print all the attributes of the blueprint
            # Additional camera parameters for better fisheye
            ...
            self.custom_attributes = {
                # Lens distortion parameters for fisheye effect
                'lens_circle_falloff': '4.0',      # Maximum falloff at edges
                'lens_circle_multiplier': '3.0',   # Strength of fisheye distortion
                'lens_k': '-0.8',                 # Barrel distortion
                'lens_kcube': '0.3',              # Additional cubic distortion
                'lens_x_size': '0.08',            # Horizontal lens size
                'lens_y_size': '0.08',            # Vertical lens size
                
                # Camera parameters
                'focal_distance': '100.0',         # Focus distance in cm
                'blur_amount': '0.5',             # Motion blur strength
                'blur_radius': '1.0',             # Atmospheric scattering    # Enable post-processing
            }

            # Apply all custom attributes
            for attr_name, attr_value in self.custom_attributes.items():
                if bp.has_attribute(attr_name):
                    bp.set_attribute(attr_name, attr_value)
                else:
                    print(f"Warning: Attribute '{attr_name}' not found in blueprint '{bp.id}'")
